...
class BrytonClient:
    def __init__(self, base_url: str = BRYTON_BASE_URL, timeout: int = 30, debug: bool = False) -> None:
    ...
    def login(self, email: str, password: str, login_pwd_key: str | None = None) -> tuple[str, str]:
        ...
        resp = self.session.post(f"{self.base_url}/api/login", json=payload, timeout=self.timeout)
        if not resp.ok:
    ...
    def auth_headers(self) -> dict[str, str]:
        ...
        return {"X-User-Id": self.user_id, "X-Auth-Token": self.auth_token}
    ...
    def find_fit_url(self, obj: Any) -> str | None:
        if isinstance(obj, dict):
            ...
            preferred_keys = ("fitUrl", "fit_url", "fileUrl", "file_url", "downloadUrl", "download_url", "url", "fit", "file", "path")
    ...
    def download_fit(self, file_url: str, output_path: Path, detail: dict[str, Any] | None = None) -> Path:
        ...
        resp = self.session.get(file_url, headers=headers, timeout=self.timeout)
        if not resp.ok:
